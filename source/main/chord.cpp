...
void Chord::print_stats()
{
	...
	fout << cardinal_change << " (" << round((double)cardinal_change / (count - 1) * 10000.0) / 100.0 << "%)\n\n";
	...
	for(int i = 0; i < count; ++i)
	{
		...
		if(i != 0)
		{
			...
			else  temp2 = (*ptr)[i].get_tension() - (*ptr)[0].get_tension();
			...
			else  temp1 = (*ptr)[i].get_root() - (*ptr)[0].get_root();
			...
			else  temp1 = (*ptr)[i].get_g_center() - (*ptr)[0].get_g_center();
...
void Chord::_set_common_notes(Chord& new_chord)
{
	new_chord.common_note = intersect(new_chord.notes, notes, true).size();
}
